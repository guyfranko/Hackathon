...
class Server:
    def __init__(self, TEST):
        ...
        self.tcpThread = threading.Thread(target=self.ActivateServerTcp, args=())
    ...
    def ServerInitializer(self, host, port):
        ...
        while time.time() < self.timeToStart:
            ...
            self.serverSocketUdp.sendto(message, (self.broadcastAddr, 13117))
            time.sleep(1.5)
        playerOne = ''
        playerTwo = ''
            ...
            while time.time() < self.gametime and winnerFlag:
                if self.gameParticipants[participantsNames[0]]['time'] == self.gameParticipants[participantsNames[1]]['time']:
                    ...
                    for name in participantsNames:
                        if min(self.gameParticipants[participantsNames[0]]['time'], self.gameParticipants[participantsNames[1]]['time']) == self.gameParticipants[name]['time']:
    ...
    def ActivateServerTcp(self):
        ...
        while not self.gameStarted:
            ...
            try:
                ...
                t = threading.Thread(target=self.getPlayers, args=(client))
                threads.append(t)
                t.start()
            except:
                pass
    ...
    def getPlayers(self, player):
        ...
        try:
            ...
            if len(self.gameParticipants) < 2:
                game = True
                self.gameParticipants[player] = {"teamName": teamName, 'group_number': self.playerNumber, 'answer': None, "time": 20}
